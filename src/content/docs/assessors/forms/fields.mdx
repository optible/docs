...
Each form field has a set of configuration options that allow you to customize its appearance and behavior. These options include:

### Label
The label is the text that appears above the field when it's displayed on the form.
### Placeholder
The placeholder is the text that appears in the field when it's empty.
### Help Message
The help message is the additional information that appears below the field when it's displayed on the form.
### Required
Determines whether the field is mandatory or not.
### Validation Rules
The validation rules determine the rules that the field must follow to be valid.
### Condition
Conditions are used to determine whether a field is displayed or hidden based on the values of other fields.
### Hidden
Determines whether the field is displayed or hidden.
</Card>
...
## Email
The email field is used to collect an email address from the applicant. It's commonly used to collect contact information or to verify the applicant's identity.
...
## Horizontal Radio
Horizontal radio buttons are used to collect a single option from the applicant. It has options of adding images which can be used to display the radio button.
